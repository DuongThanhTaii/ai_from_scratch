(function () {
  ...
  function initLangToggle() {
    ...
    btn.addEventListener('click', function () {
      window.AIFSTranslations.toggleLanguage();
    });
  }
  ...
  function openModal(idx) {
    ...
    var isVi = window.AIFSTranslations.getLanguage() === 'vi';
    document.getElementById('modalPhaseNum').textContent = (isVi ? 'GIAI ĐOẠN ' : 'PHASE ') + String(p.id).padStart(2, '0');
  ...
  function renderModalLessons(p) {
    ...
    for (var i = 0; i < p.lessons.length; i++) {
      ...
      var lessonStatusTitle = userComplete ? (isVi ? 'Bạn đã hoàn thành bài học này' : 'You completed this lesson') : '';

      html += '<div class="modal-lesson' + (userComplete ? ' user-done' : '') + '">';
      html += '<span class="modal-lesson-status ' + statusClass + '"' + (lessonStatusTitle ? ' title="' + lessonStatusTitle + '"' : '') + '></span>';
      if (l.url) {
      ...
      if ((l.status === 'complete' || userComplete) && lessonPath) {
        actionHtml = '<a href="lesson.html?path=' + lessonPath + '" class="modal-lesson-read">' + (userComplete ? (isVi ? 'Xem lại' : 'Review') : (isVi ? 'Đọc bài' : 'Read')) + '</a>';
      }
      var toggleHtml = '';
      if (hasProgress && lessonPath) {
        var toggleTitle = userComplete ? (isVi ? 'Đánh dấu chưa hoàn thành' : 'Mark as not done') : (isVi ? 'Đánh dấu hoàn thành' : 'Mark complete');
        toggleHtml = '<button type="button" class="modal-lesson-toggle' + (userComplete ? ' done' : '') + '" data-path="' + lessonPath + '" title="' + toggleTitle + '" aria-label="' + toggleTitle + '">' + (userComplete ? '✓' : '+') + '</button>';
      }
    ...
    if (hasProgress && p.lessons.length > 0) {
      ...
      if (progEl) {
        ...
        progEl.innerHTML = '<span class="modal-progress-count">' + userDone + ' / ' + p.lessons.length + '</span> <span class="modal-progress-label">' + (isVi ? 'đã hoàn thành' : 'completed') + '</span> <span class="modal-progress-pct">' + pct + '%</span>';
      }
      if (barEl && barFill) {
        barEl.style.display = '';
